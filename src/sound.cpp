...
void soundLoop() {
  ...
  if(rate>=100){
    ...
    samples_consumed=PLAY_BUF_SIZE*rate/100;
    ...
    for(int j=0;j<PLAY_BUF_SIZE;j++){
      ...
      if(rest == 0){
        ...
        play_buffer[j]=avasbytes[si]*7/10;;
      }else{
        
        play_buffer[j]=(char) ((avasbytes[si]+(avasbytes[sind(i+index+1)]-avasbytes[si])*((uint16_t)rest)/100)*7/10);;
      }
